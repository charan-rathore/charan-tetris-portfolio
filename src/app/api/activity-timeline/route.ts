...
const url = "https://api.github.com/search/issues?q=is%3Apr+author%3Acharan-rathore+is%3Amerged&sort=updated&order=desc&per_page=100";
...
let lastGood: Event[] = fallback.events as Event[];
...
async function refresh() {
  ...
  const next = data.items.flatMap((item):Event[] => {
    ...
    if (!date || !item.title || !item.html_url || !repo) return [];
