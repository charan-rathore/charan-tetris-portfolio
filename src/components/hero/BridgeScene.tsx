...
export function BridgeScene({ color, mode }: { color: string; mode: number }) {
  ...
  useEffect(() => {
    ...
    camera.position.set(9, 8, 14); camera.lookAt(0, 0, 0);
    ...
    const material = new THREE.MeshStandardMaterial({ color, emissive: color, emissiveIntensity: .25, roughness: .25, metalness: .45 });
    ...
    const exits = [new THREE.Vector3(4,2,-2),new THREE.Vector3(5,0,1),new THREE.Vector3(2,0,5)];
    const route = [new THREE.Vector3(-5,0,0),new THREE.Vector3(0,0,0),exits[mode]];
    const pathGeometry = new THREE.BufferGeometry().setFromPoints(route);
    const pathMaterial = new THREE.LineBasicMaterial({color, transparent:true, opacity:.6});
    scene.add(new THREE.Line(pathGeometry,pathMaterial));
    const signal = new THREE.Group(); scene.add(signal);
    const signalGeometry = new THREE.BoxGeometry(.42,.42,.42);
    const signalMaterial = new THREE.MeshStandardMaterial({color,emissive:color,emissiveIntensity:1.5,roughness:.2});
    [[-.45,0],[0,0],[.45,0],[0,.45]].forEach(([x,y])=>{const cube=new THREE.Mesh(signalGeometry,signalMaterial);cube.position.set(x,y,0);signal.add(cube);});
    const lamp = new THREE.PointLight(color,12,8); signal.add(lamp);
    const exitGeometry = new THREE.BoxGeometry(1.6,.12,1.6);
    const exitMaterial = new THREE.MeshStandardMaterial({color,emissive:color,emissiveIntensity:.7});
    const destination = new THREE.Mesh(exitGeometry,exitMaterial);destination.position.copy(exits[mode]);destination.position.y-=.6;scene.add(destination);
    ...
    const render = (now: number) => {
      ...
      signal.position.lerpVectors(route[segment],route[segment+1],(progress% .5)*2);
      signal.rotation.y = reduced.matches ? 0 : Math.sin(progress*Math.PI)*.5;
