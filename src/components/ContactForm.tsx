...
export function ContactForm() {
  ...
  return <form className="contact-form" onSubmit={submit} onFocus={() => { if (!began.current) began.current = Date.now(); }}>
    ...
    <label>Your message<textarea name="message" required minLength={10} maxLength={5000} rows={4} placeholder="An idea, a question, or the next thing we could build…" /></label>
